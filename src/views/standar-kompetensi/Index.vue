    ...
    headerData () {
      if (this.user.sinergi) {
        return [
          ...
          { text: 'Kelompok Jabatan', value: 'kelompok_jabatan' },
          { text: 'Urusan Pemerintah', value: 'urusan_pemerintah' },
          { text: 'Kode Jabatan', value: 'kode_jabatan' },
          { text: 'ID Instansi', value: 'id_opd' },
 ...
 Mohon untuk tidak merubah atau menghapus teks <span class="red--text">\${nomorsurat}\, \${tanggal}\, \${nip}\, \${ttdelektronik}\, \${namalengkap}\, \${pangkat}\ dan \${catatan_tte}\ </span> pada template surat !`
    ...
    _loadData (abort, opd) {
      ...
      if (this.datas.length === 0 || abort) {
        this.isLoading = true
        this.getSKJ({ search: this.searchQuery, add: this.filterTask, ...this.options })
          .then((data) => {
